...
We will now create an ensemble model that combines the user-based collaborative filtering, item-based collaborative filtering, and matrix factorisation models. We will then evaluate the accuracy of the ensemble model using RMSE. The ensemble model will be created using the following formula:

\[
\text{Ensemble Prediction} = \frac{\text{UBCF Prediction} + \text{IBCF Prediction} + \text{MF Prediction}}{3}
\]
```{r, echo = FALSE}
