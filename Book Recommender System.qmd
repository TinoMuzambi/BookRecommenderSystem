...
# Sample 50 000 ratings
set.seed(50731)
comb.ratings <- comb.ratings[sample(nrow(comb.ratings), 50000),]
```
...
Let's make some predictions using the user-based collaborative filtering system and calculate the RMSE.
```{r}
rmse <- function() {
