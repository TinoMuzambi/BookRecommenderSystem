...
```{r}
predict.rating.ubcf(59494, 616)
predict.rating.ibcf(59494, 616)
...
```{r}
ensemble.predict <- function(user.id, isbn) {
  ubcf.prediction <- predict.rating.ubcf(user.id, isbn)
  ibcf.prediction <- predict.rating.ibcf(user.id, isbn)
...
```{r}
ensemble.predict(59494, 616)

# ensemble.predictions <- mapply(ensemble.predict, test.data$User.ID[1:10], test.data$ISBN[1:10])
# sqrt(mean((ensemble.predictions - test.data$Book.Rating[1:10])^2))
```
