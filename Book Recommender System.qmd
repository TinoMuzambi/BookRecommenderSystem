...
comb.ratings <- comb.ratings %>% 
  left_join(user.ids) %>% 
  left_join(book.ids) %>% 
  select(User.ID = new.user.id, ISBN = new.book.id, Book.Rating, Year.Of.Publication, Age)
```
...
We will now build a matrix factorisation system using recosystem. 
```{r}
...
```{r}
predict.rating.ubcf(59494, "059035342X")
predict.rating.ibcf(59494, "059035342X")
predict.rating.mf(59494, "059035342X")
```
